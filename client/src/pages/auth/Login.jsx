...
export default function Login() {
  ...
  const handleFormSubmission = async (e) => {
    ...
    if (!formData.email || !formData.password) {
      return toast.error('Please enter both your email and password.');
    }

    // Call global Zustand authentication transaction script
    const outcome = await login(formData.email, formData.password);

    if (outcome.success) {
      toast.success('Session verified successfully! Welcome back.');
      navigate('/');
    } else {
